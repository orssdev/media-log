...
export default function MediaCounter(prop: Props) {
    ...
    return (
        <div className="w-(--mc) md:h-(--mc) min-h-[200px] flex flex-col justify-center items-center gap-[50px] font-bs">
            <span className="flex flex-col justify-center items-center gap-[25px] cursor-pointer " onClick={handleClick}>
                <h1 className="md:text-[56px] text-[48px]">{prop.media}</h1>
                <h2 className="md:text-[48px] text-[36px]">{prop.count}</h2>
            </span>
            {session && (
